...
CARD_WIDTH = SCREEN_WIDTH * .067
CARD_HEIGHT = SCREEN_HEIGHT * .1484
...
CARD_VERTICAL_OFFSET = CARD_HEIGHT * 0.15
...
class MyGame(arcade.Window):
    ...
    def setup(self):
        ...
        pile = arcade.SpriteSolidColor(int(MAT_WIDTH*1.3), MAT_HEIGHT, arcade.csscolor.DARK_OLIVE_GREEN)
        pile.position = START_X + (MAT_WIDTH + MAT_WIDTH * .25), BOTTOM_Y
        self.pile_mat_list.append(pile)
        ...
        for card_suit in CARD_SUITS:
            for card_value in CARD_VALUES:
                card = Card(card_suit, card_value, 1)
        ...
        for pile_no in range(PLAY_PILE_1, PLAY_PILE_7 + 1):
            ...
            for j in range(pile_no - PLAY_PILE_1 + 1):
                ...
                card.position = self.pile_mat_list[pile_no].position[0], self.pile_mat_list[pile_no].position[1] - CARD_VERTICAL_OFFSET * (j)
        ...
        if len(cards) > 0:
            ...
            if pile_index == BOTTOM_FACE_DOWN_PILE:
                ...
                for i in range(3):
                    ...
                    card.position = self.pile_mat_list[BOTTOM_FACE_UP_PILE].center_x+((i-1)*CARD_VERTICAL_OFFSET), \
                                    self.pile_mat_list[BOTTOM_FACE_UP_PILE].center_y
    ...
    def on_mouse_release(self, x: float, y: float, button: int, modifiers: int):
        ...
        # closest_card, distance = arcade.get_closest_sprite(self.held_cards[0], self.card_list)
        # for index, piles in enumerate(self.piles):
        #     if closest_card in piles:
        #         if self.pile_mat_list[index] == pile:
        #             pile = closest_card
        reset_position = True
